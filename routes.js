...
router.get('/questoes/:grande_area/:materia/:nivel', async (req, res)=>{
        ...
        try{
            ...
            if(grande_area != null && materia == null && nivel == null){

                const busca_area = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
                questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where ga.id = ${grande_area}`

                return res.status(200).json(busca_area) 
            }
            if(grande_area == null && materia != null && nivel == null){

                const busca_materia = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
                questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where m.id = ${materia}`

                return res.status(200).json(busca_materia)
            }
            if(grande_area == null && materia == null && nivel != null){
                
                const busca_nivel = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
                questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where q.nivel = ${nivel}`

                return res.status(200).json(busca_nivel)
            }
            if(grande_area != null && materia != null && nivel == null){

                const busca_gm = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
                questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where ga.id = ${grande_area} 
                and m.id = ${materia}`
            ...
            if(grande_area == null && materia != null && nivel != null){
                ...
                const busca_mn = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
                questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where m.id = ${materia} 
                and q.nivel = ${nivel}`
            ...
            if(grande_area != null && materia == null && nivel != null){
                ...
                const busca_gn = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
                questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where ga.id = ${grande_area} 
                and q.nivel = ${nivel}`
            ...
            const busca = await sql`select * from grande_area as ga inner join materia as m on m.id_area = ga.id inner join 
            questao_materia as qm on m.id = qm.id_materia inner join questao as q on m.id = qm.id_materia where ga.id = ${grande_area} 
